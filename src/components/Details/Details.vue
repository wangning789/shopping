...
<!-- 商品评论 -->
        <van-tab title="商品评论">
        <div>
            <div v-if="arr.length===0" class="wu">暂无评价</div>
             <div class="minmin" v-for="(item,index) in arr" :key="index">
              <div class="min">
            <div class="min1">
            ...
            <div class="min2">
              ...
              <div v-if="item.anonymous===false">{{arr1[0].user[0].nickname}}</div>
              <div>
                <van-rate v-model="item.rate" :size="15" color="#ffd21e" void-icon="star" void-color="#eee"/>
              </div>
            </div>
            <div class="min3">{{item.comment_time}}</div>
          </div>
         <div  class="min4">
              <div  class="min5">
              <div>内容：</div>
      ...
      <van-goods-action>
        ...
        <van-goods-action-icon icon="cart-o" :badge="badge" text="购物车" @click="clicktocat" />
...
clicktocat(){
  this.$router.push("/Shoppingcat")
    ...
    //    立即购买
    // 没登陆
    clickbuy(){this.$utils.checkLogin(this.clickbuy1)},
    // 登陆
    clickbuy1() {
      ...
      this.$api
        ...
        .then(res => {
          ...
          this.arr = res.goods.comment;
            ...
            // 最近浏览
    let b=localStorage.getItem("usa")        
    let a=localStorage.getItem("browse")
    this.user=b+"browse"
    if(this.user===null){
      this.vex.push(this.msg)
      localStorage.setItem(this.user,JSON.stringify(this.vex))
    }else if(this.user){
     this.some=JSON.parse(localStorage.getItem(this.user))
    this.some2.push(this.msg)
   this.some3=this.some2.concat(this.some)
     localStorage.setItem(this.user,JSON.stringify(this.some3))
    ...
    // 点击收藏
    // 没登陆
    clickxdd(){this.$utils.checkLogin(this.clickxdd1)},
    // 已经登陆
    clickxdd1() {
      this.num = 1;
      this.$api
    ...
    // 点击加入购物车
  Clickjiaru(){this.$utils.checkLogin(this.Clickjiaru1)},

    Clickjiaru1() {

      this.getDatacat(this.id);
     
      if(this.badge===this.cgs){
        this.badge++
      }else if(this.badge>this.cgs){
        this.badge
      }
    },
    //   加入购物车
