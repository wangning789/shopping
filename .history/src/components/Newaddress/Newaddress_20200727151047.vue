 ...
 export default {
   ...
   methods: {
    ...
    // 新增地址请求
      ...
      this.$api
        .Increase({name:name,tel:tel,address:address,isDefault:isDefault,province:province,city:city,county:county,addressDetail:addressDetail,areaCode:areaCode,id:this.id})
        .then(res => {
            if(res.code===0){
                Toast(res. msg)
            }else{
