<template>
  <div>
   ...
   <!-- 顶部导航 -->
    ...
    <!-- 中间显示部分 -->
      <div class="box3">
        ...
        <div class="box4" @click="clickon(item,index)">
   ...
   顶部导航栏
<van-tabbar v-model="clickop1">
  <van-tabbar-item icon="home-o" @click="clickop">商城</van-tabbar-item>
...
export default {
  ...
  mounted() {
  this.wgg= this.$route.query.wcg
  this.id=this.$route.query.id
    this. getData1()
    this.getData()
   
  if(this.$route.query.wcg){
    this.activeKey=this.wgg
  }

  if(this.$route.query.wcg){
    this.$api.HomeData().then(res=>{
        this.arr2=res.data.category
        let id=this.arr2[this.wgg].bxMallSubDto[0].mallSubId
        this.getData(id)
    }).catch()
  }else{
     this.$api.HomeData().then(res=>{
