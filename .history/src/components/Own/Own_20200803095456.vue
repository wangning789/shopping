  ...
  methods: {
    ...
    clickcbb() {
     console.log(this.year)
        // this.getdata(this.value3,this.year,this.moath,this.day,this.msg._id,this.value2)
    ...
    // 修改用户信息
    getdata(gender,year, month,day,id,nickname){
        this.$api.ModifyData({gender:gender,year:year, month:month,day:day,id:id,nickname:nickname}).then(res=>{
            if(res.code===200){
                Toast('更改成功');
                this.$router.back()
            }else{
